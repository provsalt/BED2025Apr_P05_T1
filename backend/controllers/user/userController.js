...
import { trackLoginAttempt, trackFailedLoginAttempt } from "../../models/admin/analyticsModel.js";
import { userLoginAttempts, failedLoginAttempts } from "../../config/metrics.js";
...
export const loginUserController = async (req, res) => {
  ...
  if (!user) {
    ...
    trackFailedLoginAttempt(validate.data.email, loginData).catch(err => {
      console.error('Failed to track failed login attempt:', err);
    });
    
    // Increment Prometheus metrics
    failedLoginAttempts.labels(validate.data.email.split('@')[1] || 'unknown').inc();
    userLoginAttempts.labels('failed', loginData.deviceType).inc();
  ...
  trackLoginAttempt(user.id, loginData).catch(err => {
    console.error('Failed to track successful login attempt:', err);
  });
