...
// functioon to get the full image URL for preview
function getFullImageUrl(url) {
  if (!url) return ''; // If no URL is provided, return an empty string
  // If the URL is already absolute (starts with 'http') or is a data URL (from FileReader), return as is
  if (url.startsWith('http') || url.startsWith('data:')) return url;
  // Otherwise, prepend the backend URL (for relative paths from the database)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => navigate('/medical'), 1200);
      } else {
    ...
    <div className="w-full p-6 bg-gray-50">
      <div className="bg-white rounded-lg shadow-sm border p-6 max-w-md mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-semibold text-gray-800">Edit Medication Reminder</h1>
          <Link to="/medical">
            <Button 
              variant="secondary" 
              size="sm"
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="imageUpload" className="text-sm font-medium text-gray-700">
              Medication Image {formData.oldImageUrl && !formData.imagePreview && ' (current image shown)'}
            </Label>
            <div className="space-y-3">
              ...
              {(formData.imagePreview || formData.oldImageUrl) && (
                <div className="relative">
                  <img
                    src={getFullImageUrl(formData.imagePreview || formData.oldImageUrl)}
